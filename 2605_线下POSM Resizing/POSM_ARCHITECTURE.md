...
| 参数 | PoC 当前值 | 说明 |
|---|---|---|
| `CAN_CX` | `6568` px（≈ 834mm） | 主体物中心在主底图的 X 坐标 |
| `CAN_CY` | `9183` px（≈ 1166mm） | 主体物中心在主底图的 Y 坐标 |
...
新版架构引入了 **动态锚点与父子依赖（Dom 树）** 的排版组（Group）结构。以下参数在 `element_layout.py` 内部硬编码或作为可配置常量读取：

| 元素（节点） | 节点类型 | 定位基准 / 计算公式 |
|---|---|---|
| 2 标题无蒙版 | **主锚点** | 宽 = `85% * 成品框宽`，左距 `4%`，顶距 `12%` |
| 1 顶部联合 logo | 伴生节点 | 宽 = `60% * 标题宽`，Y 轴在标题上方（间距 `15% * 标题高`） |
| 6 原装进口 | 伴生节点 | 宽 = `30% * 标题宽`，Y 轴在标题下方（间距 `10% * 标题高`） |
| 3 脚注 | **独立锚点** | 宽 = `85% * 成品框宽`，底距 `4%` |
| 4 右下角 logo | 伴生节点 | 宽 = `20% * 脚注宽`，与脚注底端对齐 |
